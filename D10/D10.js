...
function dice () {
  let casualnumber = Math.floor(Math.random ()*7)+1
...
let slice = deleteOne ('Buongiorno', false);
console.log(slice)
...
//function onlyLetters (str3) {
 // if (str3 === Number) {
  ...
  let letters= onlyLetters ("Ho 4 cani")
  console.log (letters)
...
function whatDayIsIt (){
  let days = ["Lunedì","Martedì","Mercoledì","Mercoledì","Giovedì","Venerdì","Sabato","Domenica"];
  let today = new Date ();
  return days [today.getDay()]
}
let dayoftoday = whatDayIsIt ()
...
//dice()
/* ESERCIZIO 9
  Scrivi una funzione chiamata "howManyDays" che riceve una 
  data come parametro e ritorna il numero di giorni trascorsi da tale data.
*/
function howManyDays (date) {
  let startingDate = new Date (date);
  let todayDate = new Date ();

  let differenceDate = startingDate - todayDate;
 // calculateDate = Math.floor()
  let calculateDate = differenceDate / (24 * 3600 * 1000) //calcolo in millisecondi
  return (Math.floor(calculateDate))
}

let daysPassed= howManyDays ("2023-4-27")
console.log (daysPassed) //undefined da capire cosa manca
...
function newestMovie (movies){
 // if (movies.lenght === 0){
  //  return null
 // }
 let inizialize = movies [0]
 for (let i= 1; i < movies.length; i++) {
  if (movies[i].Year > inizialize.Year) {
    inizialize = movies [i]
  }
  return inizialize
...
function onlyInLastMillennium (){
  for (let i = 0; index < movies.length; i++) {
...
/* ESERCIZIO 16
  ...
  la somma di tutti gli anni in cui sono stati prodotti i film contenuti nell'array "movies" fornito.
*/
...
/* ESERCIZIO 17
  ...
  che riceve una stringa come parametro e ritorna i film nell'array "movies" fornito che la contengono nel titolo.
*/
...
/* ESERCIZIO 22
  Scrivi una funzione che, tramite un ciclo, stampa 
  in console il testo contenuto in ogni tag <td> all'interno della pagina.
*/
function text (){
...
/* ESERCIZIO 24
  Scrivi una funzione per aggiungere 
  un nuovo elemento alla lista non ordinata con id "myList".
*/
function list () {
...
*/
function clearAll () {
  let clear = document.querySelector ('#myList');
  clear.innerHTML = " "
}
...
/* ESERCIZIO 29
  Crea una funzione chiamata "isItPrime" che riceve un numero come parametro e ritorna 
  true se il numero fornito è un numero primo.
*/
